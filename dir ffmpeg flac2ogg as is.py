...
to .ogg files, without filtering and resampling,
in selected folder, recursively, removing built-in preview
and other potential junk, by means of `ffmpeg`_

Current script is reduced version of other flac2ogg scripts from
`The Toad's Slimy Mudhole`_

Current script supports commandline arguments.

Run::

    python "ffmpeg flac2ogg as is.py" "target_name"

to open in "target_name" dir, or add::

    pythonw.exe "ffmpeg flac2ogg as is.py" "%1"
...
if source_dir == '':
    ...
    for filename in file_list:
        ...
        subprocess.run(
            f'ffmpeg.exe -loglevel quiet -i "{currentfile}" -map 0:a:? -c:a: libvorbis -aq 10 -vn -sn -map_metadata:s:0 0:s:0 -metadata comment="" -metadata encoder="" -metadata description="" -metadata copyright="" -metadata encoded_by="" "{oggfile}"',
